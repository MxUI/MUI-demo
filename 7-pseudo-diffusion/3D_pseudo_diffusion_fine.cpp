...
int main(int argc, char ** argv) {
        ...
        for ( int j = 0; j < Ny; ++j ) {
			for ( int i = 0; i < Nx; ++i ) {
                 ...
                 if (points0[i][j][k][0] <= tolerance){
                   double r_ = sqrt(pow((points0[i][j][k][1]-y_center),2)+
    ...
    mui::geometry::box<mui::demo7_config> send_region( {y0, z0}, {(y0+ly0), (z0+lz0)} );
    mui::geometry::box<mui::demo7_config> recv_region( {y1, z1}, {(y1+ly1), (z1+lz1)} );
    ...
    outputFileLeft << "\"X\",\"Y\",\"Z\",\"scalar_field\"\n";
 	for ( int k = 0; k < Nz; ++k ) {
        for ( int j = 0; j < Ny; ++j ) {
			for ( int i = 0; i < Nx; ++i ) {
                outputFileLeft << points0[i][j][k][0] << "," <<points0[i][j][k][1]<< "," << points0[i][j][k][2]<< "," << scalar_field0[i][j][k] << ", \n";
            }
        }
    }
    ...
    outputFileRight << "\"X\",\"Y\",\"Z\",\"scalar_field\"\n"; 
 	for ( int k = 0; k < Nz; ++k ) {
        for ( int j = 0; j < Ny; ++j ) {
			for ( int i = 0; i < Nx; ++i ) {
                outputFileRight << points1[i][j][k][0] << "," <<points1[i][j][k][1]<< "," << points1[i][j][k][2]<< "," << scalar_field1[i][j][k] << ", \n";
            }
        }
    }
    ...
    for ( int t = 1; t <= steps; ++t ) {
            ...
            for ( int j = 0; j < Ny; ++j ) {
                for ( int i = 0; i < Nx; ++i ) {
                    ...
                    if ((points0[i][j][k][0] - x0) > tolerance) {
                        ...
                        scalar_field0[i][j][k] += dr * (scalar_field0[(i-1)][j][k] - scalar_field0[i][j][k]);
            ...
            for ( int j = 0; j < Ny; ++j ) {
                for ( int i = 0; i < Nx; ++i ) {
                    ...
                    if ((points1[i][j][k][0] - x1) <= tolerance) {
                        ...
                        scalar_field1[i][j][k] = ifs[1]->fetch( name_fetch,
                                                                locf,
                                                                t,
                                                                spatial_sampler,
                                                                chrono_sampler);
                        ...
                        scalar_field1[i][j][k] += dr * (scalar_field1[(i-1)][j][k] - scalar_field1[i][j][k]);
        ...
        if ((t % outputInterval) == 0) {
            ...
            outputIntegration << std::to_string(t) << "," <<intFaceLD1<< "," << intFaceRD1 << "," <<intFaceLD3 << "," <<intFaceRD3 << ", \n";
            outputIntegration.close();
        }
	}
