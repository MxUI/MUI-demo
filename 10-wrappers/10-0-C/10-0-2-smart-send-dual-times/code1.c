/*****************************************************************************
* Multiscale Universal Interface Code Coupling Library Demo 10-0-2           *
*                                                                            *
...
int main(int argc, char **argv) {
	...
	char *header = "mpi://";
	char *appname = "PUSHER_FETCHER_1";
	char *interface = "/interface";
	char *footer = "\0";
	char *name_fetch = "code2Value";
	char *name_push = "code1Value";
	...
	// Define bounding box
	double local_x0 = 6.0; // local push origin box start
	double local_y0 = 0.0;
	double local_z0 = 0.0 + ((2.0/((double)mui_size))*((double)mui_ranks));
	double local_x1 = 7.0; // local push origin box end
	double local_y1 = 1.0;
	double local_z1 = 0.0 + ((2.0/((double)mui_size))*((double)mui_ranks+1));
	double local_x2 = 4.0; // local fetch origin box start
	double local_y2 = 0.0;
	double local_z2 = 0.0 + ((2.0/((double)mui_size))*((double)mui_ranks));
	double local_x3 = 5.0; // local fetch origin box end
	double local_y3 = 1.0;
