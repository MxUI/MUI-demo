...
int main( int argc, char ** argv ) {
    ...
    double u1[N], u2[N];
	std::string fetch_name = "u0";
	std::string push_name = "u";
    ...
    for ( int iter = 1; iter <= 1000; ++iter ) {
        printf( "Left grid itertion %d\n", iter );
            ...
            mui::point1d push_point(4);
            interface.push( push_name, push_point, u[4]);
            interface.commit( iter );

            u[6] = interface.fetch( fetch_name, 6 * H, iter, s1, s2, fr );
