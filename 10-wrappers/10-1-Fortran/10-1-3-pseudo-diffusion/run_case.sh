...
if [ -n "$1" ]; then
  if [ -n "$2" ]; then
    if [ -n "$3" ]; then
	  ...
	  cmake -DCMAKE_RUNTIME_OUTPUT_DIRECTORY=../ .. -DMUI_FORTRAN_INCLUDE_DIR=$1 -DMUI_FORTRAN_LIB_DIR=$2 -DCMAKE_PREFIX_PATH=$3 $4
    else
	  # Run cmake with the provided path as the MUI include and lib directories
...
mpirun -np 2 3D-pseudo-diffusion-coarse :\
       -np 2 3D-pseudo-diffusion-fine | tee output.log
