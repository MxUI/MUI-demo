...
int main(int argc, char ** argv) {
  ...
  mui::geometry::box<mui::demo6_config> send_region( {y0, z0pr}, {(y0+ly0), (z0pr+lz0pr)} );
  ...
  mui::temporal_sampler_exact<mui::demo6_config> temporal_sampler;
  ...
  outputFileLeft << "\"X\",\"Y\",\"Z\",\"scalar_field_A\"\n";
  for (int k = 0; k < Nz; ++k) {
    for (int j = 0; j < Ny; ++j) {
      for (int i = 0; i < Nx; ++i) {
        outputFileLeft << points0[i][j][k][0] << "," << points0[i][j][k][1]
            << "," << points0[i][j][k][2] << "," << scalar_fieldA0[i][j][k]
            << ", \n";
      }
    }
  }
  ...
  outputFileRight << "\"X\",\"Y\",\"Z\",\"scalar_field_A\"\n";
  for (int k = 0; k < Nz; ++k) {
    for (int j = 0; j < Ny; ++j) {
      for (int i = 0; i < Nx; ++i) {
        outputFileRight << points1[i][j][k][0] << "," << points1[i][j][k][1]
            << "," << points1[i][j][k][2] << "," << scalar_fieldA1[i][j][k]
            << ", \n";
      }
    }
  }
